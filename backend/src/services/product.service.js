...
//     let { category, sizes, minPrice, maxPrice, minDiscount, sort, stock, pageNumber, pageSize, color } = reqQuery;
...
//     if (category) {  // this is for same category products 
...
//         // query.query part means it adds a condition to the database query.
//         query.query.where("sizes.name").in([...sizesSet]);//Filters for any products that have a size matching the values in sizesSet.
//     }
...
//             query = query.where("quanity").gt(1);
//         }
//     }
...
//     // Skip: Ignore a certain number of products based on the calculated skip value.
//     // Limit: Fetch only the number of products specified by pageSize.
